...
class Stock:
    def __init__(self, symbol, name, shares):
        if not all((symbol, name, shares)):
            raise ValueError("Symbol, name, and shares are required.")
        if shares < 0:
    ...
    def shares(self, value):
        raise ValueError("Changing shares directly is not allowed. Use buy() or sell() methods instead.")
